...
module Pump
  module Activities
    class Cycle
      ...
      def submit!
        puts "  · Submitting #{@count} rides..."
        return nil if @count == 0
        activity = Pump::Activity.new(@login, "travel", decorate_ride(@rides.first), false).json
        puts activity
      ...
      def decorate_ride(ride)
        ...
        data["content"] = ride[:message] if ride.is_a?(Hash)
        data["published"] = Pump::Util::DateTime.json_datetime(ride[:datetime]) unless ride[:datetime].nil?
